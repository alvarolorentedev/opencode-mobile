  ...
  return (
    <Card mode="contained" style={[styles.card, { backgroundColor: palette.surface }]}> 
      <Card.Content style={styles.section}>
        ...
        <NumericSlider label="Chats per page (workspace)" minimum={5} maximum={100} step={5} value={chatPreferences.chatsPerPage} valueLabel={`${chatPreferences.chatsPerPage}`} onValueChange={(chatsPerPage) => updateChatPreferences({ chatsPerPage })} palette={palette} />
        <HelperText type="info">Paginate the workspace Chats list. Lower values load fewer rows per page, keeping the list fast for very large sessions databases.</HelperText>
        <SettingSwitchRow description="Hide subagent chats from the workspace list. They are still created and run; only the row is filtered out." onValueChange={(value) => updateChatPreferences({ hideSubagentChats: value })} palette={palette} title="Hide subagent chats" value={chatPreferences.hideSubagentChats} />
